...
1. **Profiling:** Open the Excel/CSV, validate headers, and normalize DOB to `DDMMYYYY` (8 digits, no separators). Convert common inputs like `DD/MM/YYYY` or `YYYY-MM-DD` into that exact 8-digit string before import.
...
2. **Access code on entry:** Require a short-lived admin access code (env-driven, rotated daily) before showing the admin login modal; store attempts in the audit log.
...
- **Doc templates:** Store DOCX/Markdown templates per document type; render via backend service (e.g., docx templating) and return signed PDFs. Validate and sanitize all dynamic fields (allowlist expected fields, escape user input, and use parameterized rendering) to prevent template/code injection.  
